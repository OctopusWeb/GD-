function controllerArea(viewer){
	...
	drawAreaJson1("src/assets/data/chinas.json",Cesium.ColorGeometryInstanceAttribute.fromColor(Cesium.Color.WHITE.withAlpha(0.001)));
	mouseEvent();
...
//	    }, Cesium.ScreenSpaceEventType.WHEEL);
	    handler.setInputAction(function (movement) {
	    	var pickedObject = scene.pick(movement.position);
	   		if (Cesium.defined(pickedObject)) {
		        ...
		        viewer.camera.flyTo({
			        destination : Cesium.Cartesian3.fromDegrees(cityCenter[pickID][1], cityCenter[pickID][0], 3000000.0)
			    });
		    }
	...
	function drawArea1(data,linecolors,num){
        ...
        for (var i = 0; i < boundaries.length; i++) {
            ...
            var geometryInstance = new Cesium.GeometryInstance({
                ...
                id: data.adcode + "_" + i+"-"+num
            });

            var borderInstance = new Cesium.GeometryInstance({
        ...
        polygon1 = scene.primitives.add(new Cesium.Primitive({
            releaseGeometryInstances: false,
            geometryInstances: geometryInstances,
            appearance: new Cesium.PerInstanceColorAppearance({}),

        }));
        polygon2 = scene.primitives.add(new Cesium.Primitive({
            releaseGeometryInstances: false,
	...
	function drawArea2(data,linecolors){
        ...
        polygon3 = scene.primitives.add(new Cesium.Primitive({
            releaseGeometryInstances: true,
            geometryInstances: geometryInstances,
            appearance: new Cesium.PerInstanceColorAppearance({}),

        }));
        polygon4 = scene.primitives.add(new Cesium.Primitive({
            releaseGeometryInstances: true,
            geometryInstances: borderInstances,
            appearance: new Cesium.PolylineColorAppearance({})
        }));
	...
	function moveEvent(movement){
		...
		if (viewer.camera.getMagnitude() < 800000) {
	        return;
	   }
	...
	function unselect() {
	    if (selected) {
	        ...
	        for (var j = 0; j < ids.length; j++) {
	            ...
	            attributes.color = Cesium.ColorGeometryInstanceAttribute.toValue(Cesium.Color.RED.withAlpha(0.01));
	        }
	    }
	}
}
